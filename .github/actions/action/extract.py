...
def fetch_pull_requests_with_details():
    """
    Collect PRs + reviewers + commenters with full pagination.
    - reviewers: from /pulls/{num}/reviews (review events; includes APPROVED/CHANGES_REQUESTED/COMMENTED)
    - review_comments: from /pulls/{num}/comments (code review line comments)
    - issue_comments: from /issues/{num}/comments (PR conversation comments)
    Also return interactions for reviewer->author heatmap.
    """
    prs = get_paginated_data(f"{BASE_URL}/pulls", params={"state":"all"}, where="pulls")
    pr_data, interactions = [], []
    review_events, review_comments, issue_comments = [], [], []
    ...
    for pr in prs:
        ...
        # --- Review events (who reviewed + state) ---
        reviews = get_paginated_data(
            f"{BASE_URL}/pulls/{pr_number}/reviews",
            where=f"reviews #{pr_number}"
        ) or []
        review_users = []
        for r in reviews:
            r_user = (r.get("user") or {}).get("login")
            r_state = r.get("state")  # APPROVED/CHANGES_REQUESTED/COMMENTED
            ...
            if r_user:
                review_users.append(r_user)
                if author and r_user != author:
                    interactions.append({"reviewer": r_user, "author": author, "pr": pr_number})

        # --- Code review comments (diff line comments) ---
        rev_comments = get_paginated_data(
        ...
        for c in rev_comments:
            ...
            if c_user:
                rev_comment_users.append(c_user)
                if author and c_user != author:
                    interactions.append({"reviewer": c_user, "author": author, "pr": pr_number})

        # --- Issue (conversation) comments on PR ---
        iss_comments = get_paginated_data(
        ...
        for c in iss_comments:
            ...
            if c_user:
                issue_comment_users.append(c_user)
                if author and c_user != author:
                    interactions.append({"reviewer": c_user, "author": author, "pr": pr_number})

        reviewers = sorted(set(review_users + rev_comment_users))
        commenters = sorted(set(issue_comment_users))

        total_reviews = len(reviews)
        total_review_comments = len(rev_comments)
        total_issue_comments = len(iss_comments)
...
def plot_pr_timeline(pr_df):
    ...
    plt.xlabel('Date'); plt.ylabel('Number of PRs')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.show()
...
def plot_prs_per_day(pr_df):
    ...
    plt.xlabel("Date"); plt.ylabel("PR count")
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.show()
...
def plot_time_to_merge(pr_df):
    ...
    plt.xlabel("Days"); plt.ylabel("PR count")
    plt.tight_layout()
    plt.show()
